 ...
 public function customer_list(){
   ...
   public function todays_invoice(){
        ...
        public function customer_dropdown()
    {
        ...
        $query_customer_name = $this->db->select('*')
                          ->from('customer_information')
                          ->group_start()
                          ->like('customer_name', $customer_id)
                        //   ->or_like('customer_mobile', $customer_id)
                        //   ->or_like('phone', $customer_id)
                        //   ->or_like('contact', $customer_id)
                          ->group_end()
                          ->limit(30)
                          ->get();
                          if ($query_customer_name->num_rows() > 0) {
                            $result['data'] = $query_customer_name->result_array();
                            $result['type'] = 'name';
                            return $result;  
                          }
        $query_customer_phone = $this->db->select('*')
            ->from('customer_information')
            ->group_start()
            ->like('phone', $customer_id)
            ->group_end()
            ->limit(30)
            ->get();
            if ($query_customer_phone->num_rows() > 0) {
                $result['data'] = $query_customer_phone->result_array();
                $result['type'] = 'phone';
                return $result;  
            }
            
            $query_customer_contact = $this->db->select('*')
                ->from('customer_information')
                ->group_start()
                ->like('contact', $customer_id)
                ->group_end()
                ->limit(30)
                ->get();
                if ($query_customer_contact->num_rows() > 0) {
                    $result['data'] = $query_customer_contact->result_array();
                    $result['type'] = 'contact';
                    return $result;  
                }
                          return false;
...
public function invoice_taxinfo($invoice_id){
    ...
    public function update_invoice() {
       ...
       if($multipaytype && $multipayamount){
           var_dump($multipaytype);
           
                //reconfigure multipaytype
                if($this->input->post('multipaytype') && $this->input->post('pamount_by_method')){
                    if(!empty($this->input->post('multipaytype')) && !empty($this->input->post('pamount_by_method'))){
                        //check if grand total price is not equal to sum of pamount_by_methods
                        $sum_of_pamount_by_methods = 0;
                        foreach($this->input->post('pamount_by_method') as $tmp_amt){
                            $sum_of_pamount_by_methods += (double) $tmp_amt;
                        }
                        
                        var_dump($sum_of_pamount_by_methods);
                        
                        if((double) $this->input->post('grand_total_price') !=$sum_of_pamount_by_methods){
                            $remaining_amount = (double) $this->input->post('grand_total_price') - $sum_of_pamount_by_methods;
                            
                            if($remaining_amount>0){
                                
                                var_dump($remaining_amount);
                                
                                //get Account Receivable HeadCode
                                $predefinedSupplierCode = $this->db->select('*')->from('acc_predefine_account')->get()->row()->customerCode;
                                
                                if(isset($predefinedSupplierCode)){
                                    $multipaytype[] = $predefinedSupplierCode;
                                    $multipayamount[] = (double) $remaining_amount;
                                }
            ...
            if ($multipaytype[0] == 0) { 
                ...
                $COAID      = $predefine_account->customerCode;
                if(isset($predefinedSupplierCode)){
                        if($reVID==$predefinedSupplierCode){
                            $Comment = "Due Amount for Customer";
                        }
                $subcode    = $this->db->select('*')->from('acc_subcode')->where('referenceNo', $customer_id)->where('subTypeId', 3)->get()->row()->id;
