  ...
  if($finyear<=0){
   ...
   // Journal voucher
    ...
    public function cash_balance(){
      ...
      $this->permission1->method('cash_balance','read')->access();
      $data['title']   = display('cash_balance');
      $data['module']  = "account";
      $data['cash_balance']  = $this->accounts_model->get_cash_balance();
      
      $data['company'] = '';
      $data['setting'] = $this->accounts_model->setting();
      $data['page']    = "cash_balance";
      echo Modules::run('template/layout', $data);
  }
    ...
    public function cash_balance_report(){
    ...
    public function profit_loss_report_excel($dtpFromDate,$dtpToDate){
      ...
      $this->permission1->method('profit_loss','read')->access();
        ...
        $objPHPExcel->getActiveSheet()->SetCellValue('A3',  display('sales_revenues'));
              ...
              foreach($income['nextlevel'] as $value) {
                if(count($value['innerHead']) > 0) { 
                  ...
                  foreach($value['innerHead'] as $inner) {
                    ...
                    if($x < 2) :
                      $objPHPExcel->getActiveSheet()->SetCellValue('B'. $rowCount, '');
                      $objPHPExcel->getActiveSheet()->SetCellValue('C'. $rowCount, number_format($inner['amount'],2));
                      $salesRevenue += $inner['amount'];
                    else :
                      $objPHPExcel->getActiveSheet()->SetCellValue('B'. $rowCount,  number_format($inner['amount'],2));             
                      $objPHPExcel->getActiveSheet()->SetCellValue('C'. $rowCount,  '');
                      $salesReturn += $inner['amount'];
                    endif;

                    if($inner['headName'] == 'Service Accounts') {
                      $rowCount++; 
                      $objPHPExcel->getActiveSheet()->SetCellValue('A'. $rowCount,  display('total_sales_revenues'));
                      $objPHPExcel->getActiveSheet()->SetCellValue('B'. $rowCount, '');
                      $objPHPExcel->getActiveSheet()->SetCellValue('C'. $rowCount, number_format($salesRevenue,2));
                    }

                    if($inner['headName'] == 'Sales Discounts') {
                      $rowCount++; 
                      $objPHPExcel->getActiveSheet()->SetCellValue('A'. $rowCount,  display('total_sales_returnes_and_discounts'));
                      $objPHPExcel->getActiveSheet()->SetCellValue('B'. $rowCount, number_format($salesReturn,2));
                      $objPHPExcel->getActiveSheet()->SetCellValue('C'. $rowCount, '');
                    }
        ...
        // <!-- NET SALES -->
        $objPHPExcel->getActiveSheet()->SetCellValue('A'. $rowCount,  display('net_sales'));
        $objPHPExcel->setActiveSheetIndex(0)->mergeCells('A'. $rowCount .':B'. $rowCount);
        $objPHPExcel->getActiveSheet()->SetCellValue('C'. $rowCount,  number_format(($salesRevenue - $salesReturn) ,2));
        $rowCount++;
        $rowCount++; 
        if(count($expenses) > 0) { 
          foreach($expenses as $expense) {
            $objPHPExcel->getActiveSheet()->SetCellValue('A'. $rowCount,  display('cost_of_goods_sold'));
            $objPHPExcel->setActiveSheetIndex(0)->mergeCells('A'. $rowCount .':C'. $rowCount);
                ...
                if(count($value['innerHead']) > 0) { 
                  foreach($value['innerHead'] as $inner) {
                    ...
                    if($inner['headName'] == 'Cost of Goods Sold') :
                      $objPHPExcel->getActiveSheet()->SetCellValue('B'. $rowCount, '');
                      $objPHPExcel->getActiveSheet()->SetCellValue('C'. $rowCount,   number_format($inner['amount'],2));
                      $totalCostGoods += $inner['amount'];
                    else :
                      $objPHPExcel->getActiveSheet()->SetCellValue('B'. $rowCount, number_format($inner['amount'],2));
                      $objPHPExcel->getActiveSheet()->SetCellValue('C'. $rowCount, '');
                      $totalExpense += $inner['amount'];
                    endif;
                    
                    if($inner['headName'] == 'Cost of Goods Sold') {
                      $rowCount++; 
                      $objPHPExcel->getActiveSheet()->SetCellValue('A'. $rowCount, display('total_cost_of_goods_sold'));
                      $objPHPExcel->getActiveSheet()->SetCellValue('B'. $rowCount, '');
                      $objPHPExcel->getActiveSheet()->SetCellValue('C'. $rowCount, number_format($totalCostGoods,2));

                      $rowCount++; 

                      $rowCount++; 
                      $objPHPExcel->getActiveSheet()->SetCellValue('A'. $rowCount, strtoupper(display('operating_expense')));
                      $objPHPExcel->getActiveSheet()->SetCellValue('B'. $rowCount, '');
                      $objPHPExcel->getActiveSheet()->SetCellValue('C'. $rowCount, number_format($totalCostGoods,2));
                      $objPHPExcel->setActiveSheetIndex(0)->mergeCells('A'. $rowCount .':C'. $rowCount);
                    ...
                    if($inner['headName'] == 'Employeer ICF Expense') {
                      $rowCount++; 
                      $objPHPExcel->getActiveSheet()->SetCellValue('A'. $rowCount, display('total_expense'));
                      $objPHPExcel->getActiveSheet()->SetCellValue('B'. $rowCount, number_format($totalExpense,2));
                      $objPHPExcel->getActiveSheet()->SetCellValue('C'. $rowCount, '');
                    }
        ...
        $objPHPExcel->getActiveSheet()->SetCellValue('A'. $rowCount, strtoupper(display('income_loss')));
        $objPHPExcel->setActiveSheetIndex(0)->mergeCells('A'. $rowCount .':B'. $rowCount);
        $objPHPExcel->getActiveSheet()->SetCellValue('C'. $rowCount, number_format(($salesRevenue - $salesReturn - $totalCostGoods - $totalExpense) ,2));
