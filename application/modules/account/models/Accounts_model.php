...
public function customer_service_due_vouchers($id)
	...
	{
		$this->db->select("acc_coa.HeadCode, acc_coa.HeadName, IFNULL(sum(acc_transaction.Debit), 0) Debit, IFNULL(sum(acc_transaction.Credit), 0) Credit");
		$this->db->from('acc_coa');
		$this->db->join('acc_transaction', 'acc_coa.HeadCode = acc_transaction.RevCodde and  acc_transaction.IsAppove = 1', 'left');
		$this->db->where('acc_coa.isCashNature', 1);
		$this->db->group_by('acc_coa.HeadCode');
		return  $records = $this->db->get()->result_array();     
	} 
}
