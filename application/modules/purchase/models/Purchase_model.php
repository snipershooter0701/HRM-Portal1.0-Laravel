   ...
   if (!empty($data)) {
     ...
     public function count_purchase() {
      ...
      if($this->permission1->method('manage_purchase','update')->access()){
        ...
        //enable edit for all status purchases
        if ($approve == 0 || 1==1) {
        ...
        $chalan_no ='<a href="'.$base_url.'purchase_details/'.$record->purchase_id.'">'.$record->chalan_no.'</a>';
    ...
    public function purchase_details_data($purchase_id) {
        ...
        $this->db->from('product_purchase a');
        $this->db->join('supplier_information b', 'b.supplier_id = a.supplier_id', 'left');
        $this->db->join('product_purchase_details c', 'c.purchase_id = a.id', 'left');
        $this->db->join('product_information d', 'd.product_id = c.product_id', 'left');
        $this->db->join('product_purchase e', 'e.purchase_id = c.purchase_id', 'left');
        $this->db->where('a.purchase_id', $purchase_id);
    ...
    public function insert_purchase(){
        ...
        if ($multiamnt == $paid_amount) {
            ...
            /*for ($i = 0, $n = count($p_id); $i < $n; $i++) {
                ...
                if ($value == 0) {
                    echo "product_and_supplier_did_not_match";exit;
                    ...
                    if(!empty($this->input->post('multipaytype')) && !empty($this->input->post('pamount_by_method'))){
                        ...
                        if((double) $this->input->post('grand_total_price') !=$sum_of_pamount_by_methods){
                            ...
                            if($remaining_amount>0){
                                //get supplier payable HeadCode
                                $predefinedSupplierCode = $this->db->select('*')->from('acc_predefine_account')->get()->row()->supplierCode;
                ...
                if ($multipaytype[0] == 0) { 
                    ...
                    if(isset($predefinedSupplierCode)){
                        if($reVID==$predefinedSupplierCode){
                            $Comment = "Due Amount for Supplier";
                        }
                    }
